...
new_model = tf.keras.models.load_model('lstm6.h5')
api = dType.load()
...
def dobot_get_start():
    ...
    dobot_end_x = 200 * math.cos(math.radians(15))
    dobot_end_y = 200 * math.sin(math.radians(15))
    
    dType.SetQueuedCmdClear(api)

    dType.SetHOMEParams(api, dobot_end_x, dobot_end_y, dobot_z, dobot_r, 1) # x, y, z, r 
...
def hit():
    ...
    # 현재 Dobot의 end_effector(라켓)-- x: 215 * cos(45), y: 215 * sin(45)
    #end_x = 215 * math.cos(math.radians(45))
    #end_y = 215 * math.sin(math.radians(45))

    #print("end_x: %f, end_y: %f" %(end_x,end_y))

    # 실제로 라켓 제외한 Dobot의 end_effector가 가야할 좌표
    dobot_end_x = 200 * math.cos(math.radians(15))
    dobot_end_y = 200 * math.sin(math.radians(15))
    
    # 예측 지점 or 타격 지점과 로봇 원점과의 거리 측정
    #length = (math.sqrt(math.pow(of_x,2) + math.pow(of_y,2)))

    #print("length: ", length)

    # 예측 지점을 타격할 수 있도록 Dobot end_effector 거리 조절
    # 라켓 길이(15cm) 고려하여 실제 Dobot은 15cm 덜가면 된다

    #goal_length = length - 150
    #print('goal_length: ',goal_length)

    #if goal_length > 127 and goal_length < 267:

        #offset_x = goal_length * math.cos(math.radians(45))
        #offset_y = goal_length * math.sin(math.radians(45))     
        ...
        #dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, offset_x, offset_y, dobot_z, dobot_r, isQueued = 1)

    dType.SetJOGCmd(api, 1, 2, 1)
    dType.SetWAITCmd(api, 300, 1)
    dType.SetJOGCmd(api, 1, 0, 1)
    dType.SetWAITCmd(api, 100, 1)
    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, dobot_end_x, dobot_end_y, dobot_z, dobot_r, isQueued = 1)
...
def predict_control(conn):
    ...
    while(1):    
        ...
        conn.send([len(box_points), len(box_points2)])
    ...
    while(1):
        ...
        if py != None and pz != None:
            ...
            conn.send([ball_x,ball_y,ball_y2,ball_z])
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        if box1 >2 and box2 >2:

            print('hit_count: ',hit_count)
            hit_count = int(input('hit_count: '))
